function [W, H, T, varexpl, cost]=ShiftNMF_reg(X,noc,lambda0,varargin)
%
% Algorithm for Regularized Shifted Non-negative Matrix factorization
% (rShiftNMF) adapted from code for ShiftNMF by Morten Morup, Kristoffer
% Hougaard Madsen, and Lars Kai Hansen. See "Extraction of Muscle Synergies 
% in SCI" for details 
%
% Usage:
% [W, S, T, varexpl]=shiftNMF_reg(X,noc,lambda0,opts)
%
% Input:
% X             I x J matrix
% noc           number of components
% lambda0       regularization constant for prior on delay, T
% opts. 
%    W          Initial W
%    H          Initial H
%    T          Initial T
...
while iter<maxiter & (dcost>=cost*conv_crit | mod(iter,20)==0 ) 
    ...
    if ~constT
        ...
        P.f=f;
        P.nyT=nyT;
        [T,nyT,cost,quit]=update_TmH(T,P,lambda0);
        if quit==1
            break
        end
...
% -------------------------------------------------------------------------
function [T,nyT,cost,quit]=update_TmH(T,P,lambda)
    quit=0;
    nyT=P.nyT;
    Hf=P.Hf;
    W=P.W;
    ...
    f=P.f;
    w=P.w;
    T0 = [23; 23; 30; 30; 19; 19; 20; 20; 13; 13];  %Prior on delay, T
    T0 = repmat(T0,[1,size(W,2)]);
    ...
    grad=squeeze(sum(repmat((w.*f),[size(Q,1),1,size(Q,3)]).*(conj(Q)-Q),2))+2*lambda*(T-T0); 
    for i=1:size(grad,1)
        ind=find(abs(grad(i,:))>1e-6);
        grad(i,ind)=grad(i,ind)/(-diag(Hdiag(i,ind))-squeeze(Hall(i,ind,ind)) + 1/(2*lambda));
        cond = -diag(Hdiag(i,ind))-squeeze(Hall(i,ind,ind)) + 1/(2*lambda);
        if rcond(cond) < 1e-17
            cost = 100;
            T = T0;
            quit=1;
